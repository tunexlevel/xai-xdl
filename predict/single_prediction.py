...
def diagnose_forward(reactant_smiles, target_smiles=None):
    ...
    # Unknown tokens
    unknown = [t for t in tokens if t not in token2idx]
    ...
    if target_smiles:
        print("\nExpected:")
        print(valid_smiles_or_empty(target_smiles))

    print("=" * 80)

    return result

def diagnose_forward_raw(reactant_smiles, target_smiles=None, max_len=160):

    print("=" * 100)
    print("RAW FORWARD DIAGNOSTIC")
    print("=" * 100)
    ...
    print("\nNumber of beam candidates:", len(beam_candidates))

    # --------------------------------------------------
    # SHOW RAW BEAMS
    # --------------------------------------------------
    for rank, (seq, score) in enumerate(beam_candidates[:10], 1):

        raw_tokens = decode_indices(
            seq.tolist(),
            idx2token,
            sos_idx,
            eos_idx,
        ...
        print(f"BEAM {rank}")
        print("Score:", score)
        print("Token IDs:", seq.tolist())
        print("Tokens:", raw_tokens)
        print("Raw SMILES:", repr(raw_smiles))

        # RDKit validation
        canonical = valid_smiles_or_empty(raw_smiles)
...
if __name__ == "__main__":
    ...
    main(DATASET_NAME)
    # diagnose_forward_raw('COC(=O)c1ccc(Br)c(O)c1.OCC1CC1', 'COC(=O)c1ccc(Br)c(OCC2CC2)c1', 160)
