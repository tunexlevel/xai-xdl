...
# Calculating structural validity
...
# --- 2. HELPER: Canonicalization and Validity Check ---
...
def canonicalize_smiles(smiles: str) -> str:
    ...
    Returns "" if RDKit cannot parse it.
    """
    ...
    try:
        # Note: Canonicalize implicitly checks validity
...
# *** NEW HELPER FUNCTION: Explicit Structural Validity Check ***
def is_chemically_valid(smiles_string: str) -> bool:
    """
    Checks if a SMILES string can be parsed into a chemically valid RDKit molecule object.
    
    We rely on MolFromSmiles which attempts to parse the molecular graph.
    If parsing fails (returns None or throws exception), it's invalid.
    """
    if smiles_string is None or smiles_string.strip() == "":
        return False
    try:
        # MolFromSmiles with sanitize=True attempts full chemical validation
...
# (beam_search_predict remains unchanged, as its output logic is correct)
def beam_search_predict(reactant_smiles, beam_width=10, max_len=120):
    ...
    # --- DECODE TO SMILES ---
    final_smiles_list = []
    # Note: If you implement the score dictionary, this will need to change
    # For now, we return only SMILES as the evaluate_model expects it
    for seq, score in candidates:
...
def evaluate_model():
    ...
    df_test = load_uspto_file("data/uspto50k/raw_val.csv", max_samples=100)
    ...
    # *** STRUCTURAL VALIDITY COUNTERS ***
    total_valid_top1 = 0
    total_valid_top10 = 0 # Track if at least one valid SMILES is in the Top-10
    ...
    for _, row in tqdm(df_test.iterrows(), total=len(df_test)):
        ...
        if not canon_true: 
            # Note: canon_true check implicitly ensures the ground truth is valid
        ...
        # 2. Get Top-10 Predictions (list of SMILES strings)
        ...
        # 3. Canonicalize Predictions (implicitly checking validity vs. ground truth)
        canon_preds = [canonicalize_smiles(s) for s in predicted_candidates]

        # -----------------------------------------------------------
        # *** STRUCTURAL VALIDITY CHECK (Before accuracy check) ***
        # -----------------------------------------------------------
        
        # Check Top-1 validity
        if len(predicted_candidates) > 0 and is_chemically_valid(predicted_candidates[0]):
            total_valid_top1 += 1
            
        # Check Top-10 validity (If at least ONE is valid)
        if any(is_chemically_valid(s) for s in predicted_candidates):
            total_valid_top10 += 1
        
        # -----------------------------------------------------------
        # 4. Check Matches (Top-K Accuracy)
        total += 1
    ...
    validity_top1 = (total_valid_top1 / total) * 100
    validity_top10_at_least_one = (total_valid_top10 / total) * 100
    
    print("\n=== Structural Validity Results ===")
    print(f"Top-1 Structural Validity: {validity_top1:.2f}%")
    print(f"Top-10 (At least one valid): {validity_top10_at_least_one:.2f}%")
    ...
    results['validity_top1'] = validity_top1
    results['validity_top10_at_least_one'] = validity_top10_at_least_one
